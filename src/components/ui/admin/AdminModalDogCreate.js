...
export const AdminModalCreateDog = ({ handleUpdate, personType }) => {
	...
	const handleSubmit = async (event) => {
		...
		const builtFormData = { ...restFormData, addressId: address?.id || "", dogs: walkers };
		if (address) {
			try {
										...
										<>
											<Select
												options={ownerOptions
													.filter((dog) =>
														walkers.length > 0
															? walkers.length === ownerOptions.length
																? false
																: walkers.find((x) => x.id !== dog.id)
															: true
													)
													.map((dog) => {
												...
												isDisabled={walkers.length === ownerOptions.length}
											/>
											<div className="d-flex justify-content-end">
												{walkers.length === ownerOptions.length ? (
													...
													<button
														...
														disabled={walkers.length === ownerOptions.length || selectedWalker === null}
													>
														Add dog to walker
													</button>
												)}
