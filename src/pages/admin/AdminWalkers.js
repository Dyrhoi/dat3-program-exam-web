...
export const AdminWalkers = () => {
	...
	useEffect(() => {
		const doAsynchronousCall = async () => {
			const response = await API_MAIN.get("admin/people/walkers");
	...
	return (
		<>
			<section>
				<h2>Walkers</h2>
				<p>Displaying all ({people.length}) walkers.</p>
			...
			<AdminModalEdit editing={editing} personType="walker" handleUpdate={handleUpdate} />
			<AdminModalDelete deleting={editing} personType="walker" handleUpdate={handleUpdate} />
			<AdminModalCreate personType="walker" handleUpdate={handleUpdate} />
		</>
	);
};
